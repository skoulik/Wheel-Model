...
7. **A comparison against fourteen months of a real wheel account** running some forty-five names, marked to market rather than read off its cash statement. It confirms the model link by link — the entry law to within a percent, the standing inventory's distribution over depth to within five — and rejects several standard parameter conventions. It also delivers the article's most uncomfortable finding: the account's entire advantage came from the one lever this model deliberately excludes, the choice of what to buy and when, while the option machinery earned nothing distinguishable from zero. And it shows exactly which of the model's predictions a career-length track record has no power to test.
