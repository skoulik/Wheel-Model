...
Practitioners describe the strategy in comfortable terms: "you get paid to wait," "assignment just means buying a good company at a discount." These slogans contain truth, but they are not a model. (The second one has a definite answer, and [the returns section](#sec:returns) gives it a price rather than an opinion — the discount is real and it is exactly what it costs.) They give no way to answer quantitative questions: How often will I be assigned? How much stock will I end up holding in a falling market? How much capital does this strategy really consume? What is the actual return once opportunity cost is charged? Under what market conditions does the comfortable picture break down?
...
The model has one state variable. A lot's **depth** — the gap between the call strike frozen at its purchase price and the current market price — is a random walk, and every question above is a question about that walk. The first five items below are those questions answered, in the order the article reaches them; the last three are questions the model raises once it exists. The numbers are left to the sections that earn them.

1. **Entry**, both how often and how deep. A target assignment probability fixes the strike; the depth a lot enters at then follows rather than being chosen, and it is far shallower than *buying the dip* suggests.
2. **How long a lot stays**, framed as a first-passage problem. The obvious calculation misses the average by more than an order of magnitude.
3. **How much stock is held.** Standing inventory is dominated by the deep, slow, unprofitable lots, for the same reason a hospital's beds hold sicker patients than its admissions desk sees.
4. **The return, honestly accounted.** The cash a wheel throws off is not a return. Once inventory is marked to market and the surrendered upside charged for, at fair option prices the wheel is economically **indistinguishable from simply owning the stock**.
5. **Two stability boundaries, not one**: one decides whether lots come back at all, a stricter one whether the capital in them does. Both belong to the stock, not to the strategy.
6. **The finite account**, where the article's one unbounded assumption comes off. A real balance caps the strategy: capacity comes from equity, not from the broker's permission — and a levered account can be liquidated at a price the operator does not control.
...
8. **A comparison against fifteen months of a real wheel account**, marked to market rather than read off its cash statement. The spine survives it link by link; several standard parameter conventions do not, and what the account says about its own success is uncomfortable.
