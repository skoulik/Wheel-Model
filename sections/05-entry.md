...
Two settings of that dial are carried throughout: **Standard**, p\* = 20%, which leads every worked example, and **Conservative**, p\* = 10%, the more cautious setting. Against the running example's market — weekly puts, σ = 20%, total return μ = 7%, δ = 2.5%, so m = 4.5% — the first gives k\* ≈ **0.9774**, a strike about 2.3% below the market, and the second k\* ≈ **0.9655**, about 3.5% below. Because p\* is defined in the real world, the realized assignment rate *is* p\*: one put in five, or one in ten.[^eq-kstar]
