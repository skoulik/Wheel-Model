...
One caveat about β, and it cuts against the operator rather than for them. 0.5826 is a limit rather than a fixed feature of the problem: it is what the overshoot settles down to when the barrier is many steps away, so that the walk only reaches it after a long run and no longer arrives from any particular place. A lot is not in that regime. Measured in the unit that governs it — one period's step, σ·√τ_c — a fresh lot sits a distance b from its barrier of

b  =  E[x₀] / (σ·√τ_c)  =  **0.28** of one period's step

below its own call strike. A single step is several times that whole distance, so a lot usually clears the strike on its first or second one, straight from where it started, and the overshoot is set by the size of one step rather than by the limit β describes. At such short range it is larger.

β stays in the argument as a bound rather than as the answer: it fixes what the tax would be if this strategy operated where the textbook does, and the distance between that and what the tax actually is turns out to be the more interesting quantity. It earns its keep later, at [the closed form](#eq:holding-siegmund), where β and the opposite extreme bracket the exact answer from both sides — which is what makes that answer evidence rather than merely output. How much larger is settled by **Wald's identity**.
...
The ratio of mean to median is the number to carry away: **two years against eight weeks, a factor of thirteen**. It is larger here than it would be on a slower call clock — thirteen against seven on a quarterly one — because a finer grid resolves the fast lane far more than it shortens the tail. A lucky lot leaves after four weeks rather than waiting a quarter for its first chance, which moves the median by a factor of nine across that range; the mean moves by under three, since it is carried by lots whose problem is the drift rather than the sampling.
...
which lands 9% below the exact 2.10 and is the right formula to reason with. (It takes β bare; the far-barrier bound below adds the drift correction θ/4 and reads 1.93, so the two differ by a little more than a hundredth of a step.) Every term is legible: the hole, the grid tax, and the drift that has to fill it. It also makes plain why the answer is so large. The numerator is dominated by a term that has nothing to do with how deep the assignment was; and the denominator is a small difference of larger quantities, ν = μ − δ − σ²/2 = 0.07 − 0.025 − 0.02 = **0.025**, so a modest move in any of the three carries the holding time a long way.

The way to close the 9% is not a better approximation but a bracket around the exact answer. Everything in [eq:holding-siegmund](#eq:holding-siegmund) is exact except the overshoot — Wald gives the mean, E[x₀] is known, ν is a parameter — and the overshoot depends on exactly one thing: b, the distance from the barrier in steps that [the caveat above](#eq:siegmund) already put at 0.28. That function has no closed form at a general b, but it is known at both ends of the range, and a lot's b lies between them. So the overshoot lies between the two end values, and so does E[W]. The result is a bound rather than an estimate — and it is the only check on the exact 2.10 that does not run back through the same grid that produced it.
...
The size of that group is classical, and the shape of the answer follows from a single observation. Write p(h) for the chance that a walk drifting away from a barrier at distance h ever reaches it anyway. To cover 2h it must first cover h, and from there it faces the same problem over again from the same distance — so p(2h) = p(h)², and a function turning addition of distance into multiplication of probability is an exponential. Only the rate in the exponent is left, and one more observation fixes it. Whether the walk ever gets there depends on nothing except where it stands now, so that chance cannot drift up or down on its own as the walk moves: averaging p across one short step must give back the value it started from. Take the step to be Gaussian, mean |ν|·dt away from the barrier and variance σ²·dt, and write p(h) = e^{−c·h}. Then p(h) = E[p(h + step)] reduces to E[e^{−c·step}] = 1, and for a Gaussian that expectation is exp(−c·|ν|·dt + c²σ²·dt/2). Drift and variance pull opposite ways in that exponent and cancel at exactly one rate, c = 2|ν|/σ², so p(h) = exp(−2|ν|·h/σ²). Here h is the hole from before, x₀ deepened by the same grid tax, so averaging over the entry law and taking the complement gives

P( J = ∞ )  =  1 − E[ exp( −2|ν|·(x₀ + β·σ·√τ_c) / σ² ) ]    {#eq:trapped}

At σ = 40%, with μ and δ unchanged, ν = −3.5% and the closed form gives 4.1%. That formula carries β, and here the direction of β's error is the whole point. β understates the overshoot, so h comes out too shallow; a shallower hole is easier to escape; and a formula that makes escape too easy reports too few lots trapped. Running the walk itself and counting what never comes back gives **4.4% of every assignment permanently trapped**, against the closed form's 4.1% — **7.8%** low, and low in the direction that flatters the strategy. Not delayed. Trapped, with nothing in the strategy that will ever release them. The rest of the inventory recycles around them while the trapped stratum grows at λ × 4.4% ≈ **0.46 lots a year**, without bound.
...
The closed form remains the one to reason with, because it shows that only two things govern the answer — the depth a lot enters at, and the tax sitting on top of it. But that 7.8% is a shortfall of the figure, not eight points of it, and it is not the worst case. The formula is worst precisely where an operator would go to escape the problem.

Push the strike far enough out that assignment lands at no depth at all — b → 0, the safe end of the dial — and nothing is left for the entry depth to contribute, so the whole trapped fraction is the grid. There [eq:trapped](#eq:trapped) reads **2.8%** where the truth is **3.4%**, short by nearly a fifth. That truth is one of the few numbers in this article known exactly rather than computed. Measure a lot's recovery in units of one period's step and its depth follows a Gaussian random walk with drift −θ per step, θ = |ν|·√τ_c/σ = **0.024** here; a lot escapes exactly when that walk's all-time maximum reaches b, so at b = 0 the trapped fraction is the chance that maximum is zero — which [Janssen & van Leeuwaarden](#ref:janssen-vanleeuwaarden-2007) give in closed form:

P(M = 0)  =  √2·θ · exp{ (θ/√(2π)) · Σ_r ζ(1/2 − r)·(−θ²/2)^r / (r!·(2r+1)) }    {#eq:trapped-zero}

The walk above is checked against it. And the shortfall at that end is not an accident of these parameters: the ratio of the two is √2·β = **0.824**, with nothing left in it to vary. [eq:trapped](#eq:trapped) is 17.6% low there whatever the stock, the cadence or the drift — β's fingerprint again, in the one place an operator is most likely to look for shelter.
...
Repeating a fresh lot's 40% odds period after period would have said 2.5 periods — ten weeks. The correct answer is a median of eight weeks and a mean of two years, and the gap between them is not a technicality: capital is committed for the *mean*, not the median. Every lot that leaves quickly frees its capital quickly; the operator's balance sheet is dominated by the ones that don't.

Those two facts are worse together than apart. The mean is the number that commits the capital, and it is the one a book cannot show its owner until the lots carrying it have closed — which is to say, not while the decision is still live.
