...
The dividend clause inside that qualifier is a restriction of its own. The model's asset is a **dividend aristocrat**: a company whose payout rises year after year and never falls. This is a premise, not a finding — the article does not test which names qualify — and it is what licenses the constant dividend yield of [the depth section](#sec:depth), where the consequences are spelled out. A company that does cut is a different object, and it belongs with the permanent impairment that [the outlook](#sec:outlook) records as outside the model.

The name comes from an index, which makes the premise something a reader can check. S&P Dow Jones publishes a **Dividend Aristocrats** index: those members of the S&P 500 that have raised their dividend in every one of the past twenty-five consecutive years, held in equal weight and refreshed quarterly. Index funds track the list, so the class of company this article is about can be looked at rather than imagined. It also anchors the payout assumption: the index's trailing dividend yield runs around **2%**,[^aristocrat-index] which is the order of the constant yield [the depth section](#sec:depth) settles on, and roughly what this class of company actually pays. [The live-account section](#sec:live) measures how closely the real account behind this article keeps to that basket.

Two cautions come with the name, and the first is a trap. **The index's volatility is not the asset's.** A basket of several dozen names is far steadier than any one of them, because diversification cancels whatever its members do not share. But the volatility this model runs on belongs to a *single lot* — it is what carries one name's price away from one frozen strike — and it stays a single name's however many lots an account holds at once, which is [the portfolio section](#sec:portfolio)'s subject rather than an exception to it. Reading a basket's figure in its place would understate every entry depth, every holding time, and both stability boundaries. A dividend yield does survive averaging, being a weighted mean of its members'; a volatility does not. The second caution is that the rule is **backward-looking**: twenty-five years of increases is a claim about the past, and a company that cuts its dividend leaves the index after the cut rather than before it. The list records what has held, not what will — which is the premise doing its work, and also its limit.
...
Most descriptions of the wheel collapse the first two, assuming a put is always live: sell a monthly put, it expires, sell another. Every example in Parts II and III does the same, setting T = τ_p — the always-covered case — and for the Monday-to-Friday weekly pattern it is close to the truth: such a put is live five of the week's seven days. What it misses is the weekend, which is also the stretch most likely to open on a gap. The two clocks are still separate symbols because they drive different quantities: what a put pays is set by the tenor it runs for, while how often assignment can arise is set by the cadence. Shorten the tenor without touching the cadence and the operator collects less premium for the same number of chances at assignment. [The live-account section](#sec:live) is where the two come apart.
...
Keeping the tracks separate is a discipline enforced on every formula in this article, because the most natural-looking formula for this strategy's return mixes the income of one track with the capital of another, and the resulting number is both far too flattering in the early years and far too damning later.

[^aristocrat-index]: Measured by `python code/market_regime.py`, which prints each reference basket's trailing twelve-month yield and realised volatility beside the traded universe's. At the live account's window the Dividend Aristocrats index reads a 2.02% yield at 14.4% volatility, against the S&P 500's 0.99% at 17.5%. These are measurements of public indices rather than of the model, so they carry no entry in [the reproduction appendix](#sec:reproduction), which covers the formulas.
