...
The phrase is not ours, and saying whose it is turns the premise from atmosphere into something a reader can check. S&P Dow Jones publishes a **Dividend Aristocrats** index: those members of the S&P 500 that have raised their dividend in every one of the past twenty-five consecutive years, held in equal weight and refreshed quarterly. Index funds track the list, so the class of company this article is about can be looked at rather than imagined. It also anchors the payout assumption: the index's trailing dividend yield runs around **2%**,[^aristocrat-index] which is the order of the constant yield [the depth section](#sec:depth) settles on — not a stylized number but roughly what this class of company actually pays. [The live-account section](#sec:live) measures how closely the real account behind this article keeps to that basket.
