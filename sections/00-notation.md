...
| L | **Leverage**: gross exposure per unit of equity, L = u/γ_s ([eq:leverage](#eq:leverage)), so the broker's ceiling u\* = 1 is L = 1/γ_s. The leverage an account *realizes* is E[I]/A, which is generally far below what it is permitted |
...
None of the three is displaced by the finite account. Track B remains **exposure** — what the position is worth at market — while the constraint of [the constrained section](#sec:constrained) runs on a different quantity, the **equity required** to hold that exposure, which is smaller by whatever the broker lends. Equity required is a second ledger line beside the three tracks, never a replacement for one; [the strategy section](#sec:strategy) makes the distinction where the tracks are defined.
