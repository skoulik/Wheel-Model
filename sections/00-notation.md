...
- **Cross-references:** every section's H1 carries a pandoc-style anchor (`# Title {#sec:name}`), and in-prose references are written as markdown links to those anchors — e.g. `[the holding-time section](#sec:holding)`. At assembly time these become `\label`/`\ref` pairs. Never refer to another section by bare prose or by number; always link. Current anchors: sec:notation, sec:abstract, sec:introduction, sec:prior-work, sec:strategy, sec:entry, sec:depth, sec:holding, sec:inventory, sec:returns, sec:stability, sec:constrained, sec:portfolio, sec:correlation, sec:verification, sec:live, sec:outlook, sec:derivations, sec:bibliography, sec:reproduction.
- **Formula numbering:** every displayed (non-inline) formula carries a pandoc-style anchor at the end of its display line — `E[I] = λ · E[W]    {#eq:little}`. In-prose references are markdown links to those anchors, e.g. `formula [eq:little](#eq:little)`. At assembly time the display becomes a numbered `equation` environment with `\label{eq:name}`, and each reference link is replaced wholesale by `\eqref{eq:name}` — a literal "(N)" — so phrase references to read naturally with a bare number in the link's place ("formula (7)"). Never refer to a displayed formula by paraphrase or by its section alone; cite its anchor. Current anchors, in reading order within each section: eq:n, eq:excess-return (section 04); eq:wait, eq:normal, eq:phi, eq:lognormal, eq:bs-put, eq:bs-call, eq:iv, eq:kstar, eq:p-screen, eq:screen-gap, eq:gap-prob, eq:x0-def, eq:x0-law, eq:x0-mean, eq:d-mean, eq:fall-split, eq:early-exercise-call, eq:early-exercise-put (05); eq:depth-def, eq:depth-walk, eq:nu, eq:qx, eq:ccx (06); eq:siegmund, eq:wald, eq:wald-holding, eq:survival, eq:survival-step, eq:holding, eq:holding-siegmund, eq:trapped, eq:trapped-zero (07); eq:lambda, eq:little, eq:little-finite, eq:census, eq:census-finite (08); eq:income, eq:capital, eq:mark-loss, eq:giveaway, eq:econ-pnl, eq:excess, eq:levered-excess (09); eq:count-criterion, eq:basis-multiplier, eq:capital-criterion, eq:theta, eq:account-criterion (10); eq:leverage, eq:barrier, eq:first-passage, eq:survive, eq:lmax, eq:capacity, eq:lambda-eff, eq:income-capacity, eq:astar, eq:debit-growth, eq:theta-eff, eq:gmax, eq:draw, eq:capacity-lots (11).
- **Figures:** a graph replaces a table wherever the table would sample a continuous function. A figure is an image line carrying its anchor, alone in its paragraph — `![Caption.](../figures/name.svg){#fig:name}` — and the SVG is drawn by the example script that computes its numbers (or, for a schematic, by a script that draws and computes nothing), never by hand: `python -m examples --figures` redraws them all into `figures/`, and `verify_examples.py` fails if a committed figure no longer matches a fresh draw. In-prose references work like formula references: a markdown link whose text is the anchor, replaced at assembly by `\ref{fig:name}` — a bare number — so the prose supplies the word: "figure [fig:census](#fig:census)". The exact values behind a figure live in [the reproduction appendix](#sec:reproduction), whose entry for the script names the figure it draws. Current figure anchors, in reading order within each section: fig:payoff-diagrams (02); fig:early-exercise-call (05); fig:depth-exit-premium (06); fig:survival-curve (07); fig:inventory-approach, fig:depth-census (08).
...
- **Derivations:** [the derivations appendix](#sec:derivations) is the only file that may declare a `{#drv:...}` anchor, and prose reaches an entry by markdown link in the ordinary way — `[the derivation](#drv:x0-mean)`. It holds the algebra behind formulas whose statement in a section would otherwise have to be taken on trust; only the complex ones earn an entry, and a formula that is a definition, a standard object, or already derived in its own prose does not. **A derivation may never declare a `{#eq:...}` anchor**: the formula belongs to the section that states it, and a second declaration would give one formula two homes and break the register below. Displayed steps inside a derivation therefore carry no numbers at all. A derivation may cite another (the shared Gaussian lemma is cited by two), which is why an entry counts as reachable when another derivation links it.
- **Reproducibility:** every worked numerical example in the text is recomputed by `code/verify_examples.py`, which is organized section by section. If you change a formula or an example, update and re-run the script.
- **Finding the code behind a figure:** [the reproduction appendix](#sec:reproduction) lists every script, the commands that produce the article's own values, and what those commands print. It is generated, never hand-maintained, and it is reached **by formula number** — its entries name the formulas they back, so a reader who wants to check a displayed formula looks that formula up. Sections therefore carry no per-formula reproduction footnote; the few figures quoted in prose without a formula of their own link their appendix entry directly (`[their appendix entry](#repro:returns-beta)`), and `#repro:` anchors are declared only by the generated appendix. A footnote in a section is for something a reader needs at that page — what a printed line means, which lines are checks rather than readings — never for a command line.
...
| E[overshoot] | Mean depth a lot stands *past* its strike at the first expiry that counts — a depth like x₀, not a count of steps. β·σ·√τ_c is its far-barrier limit ([eq:siegmund](#eq:siegmund)); the model's own value is 0.667·σ·√τ_c = 0.037 ([eq:wald-holding](#eq:wald-holding)) |
| S_j, S(u) | Survival sequence, S_j = P(J > j) ([eq:survival](#eq:survival)); S(u) = P(W > u) is the same curve against a lot's age in years, S(u) = S_j for j·τ_c ≤ u < (j+1)·τ_c, so ∫₀^∞ S(u) du = E[W] ([eq:little-finite](#eq:little-finite)) |
| f_j | Density of depth across the lots still held after j call periods — a sub-density integrating to S_j, not to 1. Advanced one period at a time by [eq:survival-step](#eq:survival-step) |
...
| ρ(x), ρ(x; H) | **Depth census**: how the standing inventory is distributed over depth. ρ(x) is the stationary census ([eq:census](#eq:census)), which weights every call period alike; ρ(x; H) is the census over a window of H years ([eq:census-finite](#eq:census-finite)), which weights period j by the share w_j of that window in which it can be observed. They stand to each other as [eq:little](#eq:little) does to [eq:little-finite](#eq:little-finite), and for the same reason — **a census figure is ρ(x; 30) unless the stationary limit is named** |
| f | **Weighting**: what one lot accrues, per unit of time held, of whatever quantity is being tracked — a function of the lot's state, so here a function of its depth x. f = 1 counts lots; f = e^x counts the capital they tie up. See [the inventory section](#sec:inventory) |
| G, H | The weighted form of Little's law, H = λ·G, stated in [the inventory section](#sec:inventory)'s weighted-law detour. G is the total one lot accumulates at weighting f over its whole stay, averaged across lots; H is the long-run rate at which the whole book accrues that quantity. At f = 1 they are E[W] and E[I], and the law is [eq:little](#eq:little). See [the inventory section](#sec:inventory), and [the stability section](#sec:stability) for what each of them being finite decides |
| d | Fractional price drop at the moment of assignment, S → S(1−d). Not a free input: it follows from x₀ by [eq:fall-split](#eq:fall-split), and its average is [eq:d-mean](#eq:d-mean) (≈ 3.8% for the running example). Measured from the far side of the strike, unlike x₀, which is why it carries m and x₀ does not |
...
| d₁, d₂ | The two arguments of the Black–Scholes formula ([eq:bs-put](#eq:bs-put)), d₁ = d₂ + σ·√τ. N(−d₂) is the probability a put finishes in the money; the put's delta is e^(−δ·τ)·N(−d₁), and **not** N(−d₁), which is the no-dividend shorthand. **Each is evaluated at whichever drift its own formula runs on**: r − δ in a price or on the screen ([eq:bs-put](#eq:bs-put), [eq:p-screen](#eq:p-screen)), the world's m in a real-world expectation ([eq:d-mean](#eq:d-mean)). Changing the drift moves both by the same amount, since d₁ − d₂ = σ·√τ does not depend on it. See [the entry section](#sec:entry) |
