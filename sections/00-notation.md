...
None of the three is displaced by the finite account. Track B remains **exposure** — what the position is worth at market — while the constraint of [the constrained section](#sec:constrained) runs on a different quantity, the **equity required** to hold that exposure, which is smaller by whatever the broker lends. Equity required is a second ledger line beside the three tracks, never a replacement for one; [the strategy section](#sec:strategy) makes the distinction where the tracks are defined, and [the returns section](#sec:returns) tabulates the quantity — γ_p·k + γ_s·E[I], the broker lending against held shares but not against the put collateral — as a row beside the two capitals rather than as a denominator.
