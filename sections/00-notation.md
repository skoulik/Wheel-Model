...
| f | **Weighting**: what one lot accrues, per unit of time held, of whatever quantity is being tracked — a function of the lot's state, so here a function of its depth x. f = 1 counts lots; f = e^x counts the capital they tie up. See [eq:h-lambda-g](#eq:h-lambda-g) |
| G, H | The weighted form of Little's law, H = λ·G ([eq:h-lambda-g](#eq:h-lambda-g)). G is the total one lot accumulates at weighting f over its whole stay, averaged across lots; H is the long-run rate at which the whole book accrues that quantity. At f = 1 they are E[W] and E[I], and the law is [eq:little](#eq:little). See [the inventory section](#sec:inventory), and [the stability section](#sec:stability) for what each of them being finite decides |
