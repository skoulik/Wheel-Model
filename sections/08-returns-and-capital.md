...
> **[Flagged for revision — see TODO #18]** The layered simulation (`code/wheel_sim.py`, `drafts/2026-07-22-layered-simulation-findings.md`) shows the base case above is too kind: the run rate [eq:run-rate](#eq:run-rate) survives layering, but average capital comes out ≈ 5.3·S₀, not 1.37·S₀, because standing inventory is dominated by deep lots whose calls are nearly worthless — collapsing the base-case excess return from +9.9%/yr to ≈ 0. The correction is tier 2's first-passage treatment (TODO #19); until then, read the base-case excess return as an upper bound of the homogeneous approximation.
