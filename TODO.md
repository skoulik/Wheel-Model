...
4. **Volatility skew** (all that remains of the old #4). The article carries one implied volatility σ_IV and one spread over realized. Skew means the puts sold are systematically richer than the calls sold, so the edge concentrates in the put leg and in shallow lots — which matters because the break-even spread computed in [the returns section](#sec:returns) is only ~0.2 vol points, small enough that where the premium sits changes the picture. `model.Config` carries `iv_spread` as a single scalar; splitting it into put and call legs is a small change, and the honest version would also make the spread strike-dependent.
